...
- `./scripts/run-phase.sh --runtime <gemini|codex|claude|opencode> --role backend --phase <plan|implement|review> "PROMPT"`
...
- Phase-aware workflow: `plan -> implement -> review`
...
- Review branch guard (`feature/*`) before executing review phase
...
- `plan.json`, `implement.json`, or `review.json` (phase metadata)
- `preflight.json`
- `planning_debug.log`
...
| `AGENT_PHASE` | auto-detected | Forces the execution phase (`plan`, `implement`, `review`). | `AGENT_PHASE=implement ./scripts/agent.sh --runtime gemini --role backend "Proceed with implementation"` |
...
| `AGENT_ENFORCE_REVIEW_FEATURE_BRANCH` | `true` | Blocks review when current branch is not `feature/*`. | `AGENT_ENFORCE_REVIEW_FEATURE_BRANCH=false ./scripts/run-phase.sh --runtime gemini --phase review "Review the code"` |
...
| `AGENT_MCP_RETRY_ATTEMPTS` | `2` | Retry attempts for MCP auxiliary steps (Sonar/Jira). | `AGENT_MCP_RETRY_ATTEMPTS=4 ./scripts/run-phase.sh --runtime gemini --phase review "Review the code"` |
| `AGENT_MCP_RETRY_BASE_DELAY_SECONDS` | `2` | Initial delay between retries (backoff doubles this value). | `AGENT_MCP_RETRY_BASE_DELAY_SECONDS=5 ./scripts/run-phase.sh --runtime gemini --phase review "Review the code"` |
...
| `AGENT_GEMINI_MUTATING_APPROVAL_MODE` | `yolo` | Approval mode for implementation/review calls that may edit code. | `AGENT_GEMINI_MUTATING_APPROVAL_MODE=yolo ./scripts/run-phase.sh --runtime gemini --phase implement "Proceed with implementation"` |
...
| `AGENT_GEMINI_AUX_RESUME_POLICY` | `auto` | Resume behavior for Sonar/Jira aux calls (`auto`, `always`, `never`). | `AGENT_GEMINI_AUX_RESUME_POLICY=never ./scripts/run-phase.sh --runtime gemini --phase review "Review the code"`; `AGENT_GEMINI_AUX_RESUME_POLICY=always ...`; `AGENT_GEMINI_AUX_RESUME_POLICY=auto ...` |
...
### Review integrations and quality gates
...
- `gemini`: full plan/implement/review flow with MCP-aware review automation
