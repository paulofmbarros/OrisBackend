...
Usage:
  ./scripts/pre-release-guard.sh [--ticket OR-123] [--artifacts-root tmp/state/artifacts] \
    [--plan-metadata path] [--implement-metadata path] [--review-metadata path] \
    [--publish-log tmp/state/publish.log] [--smoke-log tmp/state/smoke_test.log]

Description:
  Validates required plan/implement/review artifacts and release evidence before deploy/tag.
EOF
}
